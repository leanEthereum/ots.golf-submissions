...
`2⁻¹²⁸`, 127-bit strong security, and verification within 206 compressions on every path.
-/
...
theorem cost : scheme.VerifyCostAtMost 206 := by
  apply AlgorithmAdapter.verifyCost Forest.forestScheme (v := 205) (by decide)
  intro i
  have h := Forest.forestScheme_verifyCost i
  change 1 + Forest.forestScheme.graph.reconstructCost (Forest.forestScheme.sets i) = 206 at h
  omega
...
/-- A complete admissible, strongly secure, 206-compression construction. -/
theorem certificate :
    scheme.Admissible (1 / 2 ^ 128) ∧
    scheme.Secure ∧ scheme.VerifyCostAtMost 206 :=
