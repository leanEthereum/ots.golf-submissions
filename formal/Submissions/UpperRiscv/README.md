# RISC-V upper bound: 426 cycles

A certified RV64IM verifier for a bare-chain forest one-time signature: every execution,
accepting or rejecting, terminates within 426 cycles and computes exactly the Lean verifier's
...
- **Scheme.** 28 hash chains of 32 levels under one root (`Names.lean`), with a complete Lean
  ...
  value. The index is `H(pk ‖ message ‖ nonce)` read as 28 fields in the 16-bit lanes of its
  first three words: two five-bit fields per lane of words 0 and 1 (lane bits 2 and 7) and three
  four-bit fields per lane of word 2 (lane bits 2, 6 and 10) (`Valid.lean`); it is accepted when
  the 28 fields sum to 215. Chain `k` is disclosed at position `31 - field k` (`FixedChoice.lean`), so a signature is
...
- **Machine image.** `Program.lean` is an 886-instruction RV64IM image with an 80-byte data image.
  ...
  lanes with one mask and at most one shift each (the fields sit at bit 2 of their lanes, or a
  fixed shift above it), checks the field sum with one multiplication, and
  stores `jumpBase - 4 · field` for every chain in the 56 bytes just below the signature, over the
  consumed public key and message. A chain
...
- **Cycle count.** One cycle per executed instruction and eleven for the 5440-bit root hash: 51
...
| `Verifier.lean` | `image_refines`: the image equals the certified verifier on every input, within 426 cycles |
