# RISC-V upper bound: 436 cycles

A certified RV64IM verifier for a bare-chain forest one-time signature: every execution,
accepting or rejecting, terminates within 436 cycles and computes exactly the Lean verifier's
...
- **Machine image.** `Program.lean` is a 896-instruction RV64IM image with an 80-byte data image.
  ...
  stores `jumpBase - 4 · field` for every chain in the 64 bytes just below the signature, over the
...
- **Cycle count.** One cycle per executed instruction and eleven for the 5440-bit root hash: 61
...
| `Verifier.lean` | `image_refines`: the image equals the certified verifier on every input, within 436 cycles |
