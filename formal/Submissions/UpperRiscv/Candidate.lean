...
/-- Every accepting or rejecting run costs at most 364 cycles: 40 for index processing,
303 for all chain blocks, and 21 for the root and decision. -/
theorem submission_cycles : submission.CyclesAtMost 364 := by
  intro pk m bits b cycles completed
  exact (RiscvMixedProgram.image_refines pk m bits).2 b cycles completed

/-- Every requirement of a scored RISC-V submission, at 364 cycles. -/
theorem machineCertificate : submission.Certificate 364 :=
