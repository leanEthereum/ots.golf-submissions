...
wide for the first sixteen slots, four for slots `16 … 23` and for the even slots `24, 26, 28, 30`,
none for the odd slots `25, 27, 29, 31`. Those four dead slots all sit in lane word `(3, 1)`, which
is therefore not emitted: seven lane words are summed without carries, and the sum of the four
lanes of the total is read from the top lane of a product (`topLane`). Everything reduces to
natural number arithmetic through `Nat.and_mod_two_pow` and `Nat.and_div_two_pow`.
-/
...
/-- The mask of index word `w`: five-bit fields for the first two words, four-bit fields for the
last two. -/
def maskNat (w : ℕ) : ℕ := if w < 2 then broadcast 0x7C else broadcast 0x3C

/-- The field width of lane `l` of lane word `(w, i)`: the width of slot `8 w + 2 l + i`. The dead
lane word `(3, 1)`, whose slots all have width zero, is excluded by `hwi`. -/
theorem wid_lane (w i l : ℕ) (hw : w < 4) (hi : i < 2) (hl : l < 4) (hwi : 2 * w + i < 7) :
    wid (8 * w + 2 * l + i) = if w < 2 then 5 else 4 := by
  unfold wid
  split_ifs <;> omega
...
theorem laneValue_toNat (u : Word) (w i : ℕ) (hw : w < 4) (hi : i < 2) (hwi : 2 * w + i < 7) :
  ...
  have e0 := wid_lane w i 0 hw hi (by norm_num) hwi
  have e1 := wid_lane w i 1 hw hi (by norm_num) hwi
  have e2 := wid_lane w i 2 hw hi (by norm_num) hwi
  have e3 := wid_lane w i 3 hw hi (by norm_num) hwi
  ...
  unfold maskNat
  by_cases hw2 : w < 2
  · simp only [if_pos hw2]
    interval_cases i
    · rw [if_pos rfl, BitVec.toNat_shiftLeft, Nat.shiftLeft_eq, and_mask7C]
      norm_num
      all_goals omega
    · rw [if_neg (by norm_num), BitVec.toNat_ushiftRight, Nat.shiftRight_eq_div_pow, and_mask7C]
      norm_num
      all_goals omega
  · simp only [if_neg hw2]
    interval_cases i
    · rw [if_pos rfl, BitVec.toNat_shiftLeft, Nat.shiftLeft_eq, and_mask3C]
      norm_num
      all_goals omega
    · rw [if_neg (by norm_num), BitVec.toNat_ushiftRight, Nat.shiftRight_eq_div_pow, and_mask3C]
      norm_num
      all_goals omega
