...
def maskNat (_g : ℕ) : ℕ := broadcast 0x1e3c
def laneNat (u g : ℕ) : ℕ :=
...
private theorem fold_digit (a b r : ℕ) (ha : a < 128) :
    (a+128*b+65536*r)%128 = a := by omega

private theorem fold_quotient (a b r : ℕ) (ha : a < 128) (hb : b < 512) :
    (a+128*b+65536*r)/65536 = r := by omega

/-- The shifted fields fit into alternating seven- and nine-bit cells. -/
theorem fold_fields (s0 s1 s2 s3 t0 t1 t2 t3 : ℕ)
    ...
    let q := (p+p/128)%18446744073709551616
    q/4%128 = s0+t0 ∧ q/262144%128 = s1+t1 ∧
      q/17179869184%128 = s2+t2 ∧ q/1125899906842624%128 = s3+t3 := by
  dsimp only
  let n := s0+128*t0+65536*s1+8388608*t1+4294967296*s2+
    549755813888*t2+281474976710656*s3+36028797018963968*t3
  let rest := t0+512*s1+65536*t1+33554432*s2+4294967296*t2+
    2199023255552*s3+281474976710656*t3
  have hp : preFold s0 s1 s2 s3 t0 t1 t2 t3 = 4*n := by dsimp [preFold,n]; ring
  have hn0 : n = s0+128*rest := by dsimp [n,rest]; ring
  have hn : n/128 = rest := by rw [hn0]; omega
  have hq : (4*n+4*n/128)/4 = n+n/128 := by omega
  have hb : 4*n+4*n/128 < 18446744073709551616 := by dsimp [n]; omega
  rw [hp, Nat.mod_eq_of_lt hb]
  let m3 := (s3+t3)+128*t3
  let m2 := (s2+t2)+128*(t2+4*s3)+65536*m3
  let m1 := (s1+t1)+128*(t1+4*s2)+65536*m2
  let m0 := (s0+t0)+128*(t0+4*s1)+65536*m1
  have q0 : n+n/128 = m0 := by rw [hn]; dsimp [n,rest,m0,m1,m2,m3]; ring
