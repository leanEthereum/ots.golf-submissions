...
def copyOffset (q d : ℕ) : ℕ :=
  52 + groupOffset (group q) + 128*(copies q-1-d) + 64*withinGroup q

theorem copyStart_eq (q d : ℕ) : copyStart q d = 4096+4*copyOffset q d := by
  unfold copyStart copiesStart copyOffset
  omega

theorem copyCode_length' : ∀ q : Fin 16, ∀ d : Fin (copies q), (copyCode q d).length = 64 := by
  decide +kernel

theorem group_image' : ∀ g : Fin 8,
    (verifier.drop (52+groupOffset g)).take (groupCode g).length = groupCode g := by
  decide +kernel

theorem copy_group' : ∀ q : Fin 16, ∀ d : Fin (copies q),
    ((groupCode (group q)).drop (128*(copies q-1-d)+64*withinGroup q)).take 64 =
      copyCode q d := by
  decide +kernel

theorem copy_in_group_bounds' : ∀ q : Fin 16, ∀ d : Fin (copies q),
    128*(copies q-1-d)+64*withinGroup q+64 ≤ (groupCode (group q)).length := by
  decide +kernel

theorem group_lt' : ∀ q : Fin 16, group q < 8 := by decide +kernel
...
theorem copy_located (s : MachineState) (global : Riscv.CodeAt s (W 4096) verifier)
  ...
  rw [← eg] at hg
  have hc := CodeAt.drop hg.append_left (128*(copies q-1-d)+64*withinGroup q)
  have ec := List.take_append_drop 64
    ((groupCode (group q)).drop (128*(copies q-1-d)+64*withinGroup q))
  rw [copy_group' q d] at ec
  rw [← ec] at hc
  have h := hc.append_left
  rw [W_add, W_add] at h
  have e : 4096+4*(52+groupOffset (group q))+4*(128*(copies q-1-d)+64*withinGroup q) =
