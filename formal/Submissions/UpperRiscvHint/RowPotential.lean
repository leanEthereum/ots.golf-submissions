import Submissions.UpperRiscvHint.Rows
import Submissions.UpperRiscvHint.RowIneq
...
theorem psi_step :
  ...
  have ha : ∀ m, Row.a d' m = Row.a d m + if m = m₀ ∧ i ∈ validSet then 1 else 0 :=
    fun m => by simp only [Row.a, hd', rowAcc_cacheQuery w hfresh m]; push_cast; rfl
  have hv : Row.v d' = Row.v d + if i ∈ validSet ∧ i ∉ V d then 1 else 0 := by
  ...
  have hb : ∀ m, Row.b d' m ≤
    ...
    exact_mod_cast this
  have hD : ∀ m, Row.D d' m = Row.D d m + (if m = m₀ ∧ i ∈ validSet then 1 else 0) -
  ...
  · -- an index already held
    have hlt : i ∈ validSet := h1
    have hr : Row.r d' = Row.r d := by
      simp only [Row.r, hv, if_neg (fun h : i ∈ validSet ∧ i ∉ V d => h.2 h2), add_zero]
  ...
  · -- a new accepted index
    ...
    have hr : Row.r d' = Row.r d + 1 / Row.M := by
      simp only [Row.r, hv, if_pos (⟨hlt, hnV⟩ : i ∈ validSet ∧ i ∉ V d)]
      ring
  ...
  · -- a rejected index
    have hnV : i ∉ V d := fun h => h1 (V_sub_valid d i h)
    have hr : Row.r d' = Row.r d := by
      simp only [Row.r, hv, if_neg (fun h : i ∈ validSet ∧ i ∉ V d => h1 h.1), add_zero]
    have hrow : ∀ m, Row.pr d' m ≤ Row.pr d m + if m = m₀ then
      ...
      have hx : Row.x d' m ≤ Row.x d m := by
        simp only [Row.x, hr, ha m, if_neg (fun h : m = m₀ ∧ i ∈ validSet => h1 h.2),
          add_zero]
        simp only [hhit, if_neg (fun h : m = m₀ ∧ i ∈ V d => hnV h.2), add_zero] at hbm
        linarith
      by_cases hm : m = m₀
...
theorem sum_gCls_le (m₀ : EMessage) :
  ...
  have hVsub : V d ⊆ validSet := V_sub_valid d
  have hvalid : validSet ⊆ Finset.range (2 ^ idxBits) := fun i hi =>
  ...
  have hF1 : (Finset.range (2 ^ idxBits)).filter (fun i => ¬ i ∈ validSet) =
      Finset.range (2 ^ idxBits) \ validSet := by
    ext i; simp only [Finset.mem_filter, Finset.mem_sdiff]
  have hF2 : ((Finset.range (2 ^ idxBits)).filter (fun i => i ∈ validSet)).filter
      (fun i => i ∈ V d) = V d := by
    ext i
  ...
  have hF3 : ((Finset.range (2 ^ idxBits)).filter (fun i => i ∈ validSet)).filter
      (fun i => ¬ i ∈ V d) = validSet \ V d := by
    ext i
  ...
  have hdd : ∀ m, Row.dd d m = ∑ i ∈ V d, Row.hit d m i := fun m => by
    ...
    push_cast
    rfl
  have e1 : ∑ _i ∈ Finset.range (2 ^ idxBits) \ validSet, A = (Row.I - Row.M) * A := by
    rw [Finset.sum_const, Finset.card_sdiff_of_subset hvalid, Finset.card_range, nsmul_eq_mul,
      Nat.cast_sub (show (validSet).card ≤ 2 ^ idxBits from numValid_le)]
  ...
  have e3 : ∑ _i ∈ validSet \ V d, 1 / Row.M = 1 - Row.r d := by
    rw [Finset.sum_const, Finset.card_sdiff_of_subset hVsub, nsmul_eq_mul,
      Nat.cast_sub (show (V d).card ≤ (validSet).card from card_V_le_numValid d)]
    simp only [Row.r, Row.v, Row.M, numValid]
    have : ((validSet).card : ℝ) ≠ 0 := hM.ne'
    field_simp
...
theorem psi_dom (m : EMessage) (c : ℕ) (hc1 : (rowFresh d m).card ≤ c + trials)
    ...
    ((rowBad d m).card : ℝ≥0∞) + c * (((V d).card : ℝ≥0∞) / 2 ^ idxBits) ≤
      ENNReal.ofReal (Row.θ * psi d) *
        ((rowAcc d m).card + c * ((numValid : ℝ≥0∞) / 2 ^ idxBits)) := by
  ...
  have hR : ENNReal.ofReal (Row.θ * psi d) *
      ((rowAcc d m).card + c * ((numValid : ℝ≥0∞) / 2 ^ idxBits)) =
      ENNReal.ofReal (Row.θ * psi d *
        (((rowAcc d m).card : ℝ) + c * ((numValid : ℝ) / 2 ^ idxBits))) := by
