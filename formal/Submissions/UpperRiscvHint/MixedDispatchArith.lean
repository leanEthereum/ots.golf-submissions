...
theorem baseLane_bounds (q : ℕ) (hq : q < 16) :
    15420 ≤ baseLane q ∧ baseLane q < 65536 := by
...
theorem baseWord_second : baseWord 1 = baseWord 0 := by decide +kernel
...
theorem lane_halfword (u g l : ℕ) (hg : g < 4) (hl : l < 4) (L : Word)
    ...
    (W (baseWord g)-L).toNat/2^(16*l)%2^16 =
      baseLane (4*g+l) - (4*fineFld g u l + 1024*coarseFld g u l) := by
  ...
  have e : baseWord g - laneNat u g =
      (baseLane (4*g)-(4*fineFld g u 0+1024*coarseFld g u 0)) +
      2^16*(baseLane (4*g+1)-(4*fineFld g u 1+1024*coarseFld g u 1)) +
      2^32*(baseLane (4*g+2)-(4*fineFld g u 2+1024*coarseFld g u 2)) +
      2^48*(baseLane (4*g+3)-(4*fineFld g u 3+1024*coarseFld g u 3)) := by
