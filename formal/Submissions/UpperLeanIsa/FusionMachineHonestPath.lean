import Submissions.UpperLeanIsa.FusionMachineHonestChain
...
`219` instructions at cost `1029` (`honest_run`).
-/
...
abbrev tpsF (P : Fusion.Params) (T : Tab) (f : HashTable) (pk : PublicKey) (m : Message)
    (bits : List Bool) : Fusion.Tops :=
...
theorem rootState_last (f : HashTable) (P : Fusion.Params) (tp : Fusion.Tops) :
    ∀ n r st, rootState f P tp r (n+1) st =
      ans f (P.rootInput tp ⟨(r+n)%3,Nat.mod_lt _ (by decide)⟩ (rootState f P tp r n st)) := by
  intro n
  induction n with
  | zero => intro r st; rfl
  | succ n ih =>
...
theorem RAF_eq {i : ℕ} (hi : i<3) :
    RAF P T f pk m bits i = rootState f P (tpsF P T f pk m bits) 0 (i+1) 0 := by
  unfold RAF; exact rootAnsF_spec P f _ 3 0 _ i hi
...
include hlen in
theorem rootSeq_honest {i : ℕ} (hi : i≤3) :
...
include hC hlen in
theorem honest_rootCall {r : ℕ} (hr : r<3) :
  ...
  have hmd : ∀ r : Fin 3, cellBits (hv P T f pk m bits (rootMdCell r.val)) = P.rootMd r := by
    intro r
...
include hC hlen in
/-- Every root instruction in a home block is one of the three root calls. -/
theorem honest_rootIns {u : ℕ} (hu : u < 13) :
  ...
  · simp only [List.mem_singleton] at hy
    subst y
    have hr : hcall u < 3 := by unfold hcall; split_ifs <;> omega
...
theorem honest_next {u : ℕ} (hu : u < 13) : (nextOp u).Rel f (hv P T f pk m bits) := by
  ...
  · rw [if_neg h12]
    show hv P T f pk m bits pkCell = hv P T f pk m bits (stCell 2) * hv P T f pk m bits oneCell
    rw [hv_one, mul_oneV, (hv_st (by omega)).1, show pkCell = 0 from rfl,
      hv_lt P T f pk m bits (by omega), inputWord_pk]
    have hlo : (RAF P T f pk m bits 2).extractLsb' 0 128 = pk := by
      rw [RAF_eq (by omega)]
...
theorem honest_run :
    simulateQ (unifFwdAnswerImpl f)
        (LeanIsa.runCost (program T) (LeanIsa.loadInput pk m bits (imageF P T f pk m bits)) 219
          Regs.initial) = pure (some 1029) := by
