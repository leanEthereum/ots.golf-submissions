...
theorem exists_lower_digit {m₁ m₂ : Message} (h : m₁ ≠ m₂) :
    ∃ i : Fin 34, digit m₂ i < digit m₁ i := by
  by_contra hn
  push Not at hn
...
def honestWord (f : HashTable) (sk : Words) (i : Fin 34) (j : ℕ) : Word :=
  chainValue f i.val 0 j (sk i)

/-- A second preimage for one particular step of the honest chain. -/
def ChainSecondPreimage (f : HashTable) (sk : Words) (i : Fin 34) (j : ℕ) (y : Word) : Prop :=
  j < 255 ∧ y ≠ honestWord f sk i j ∧
    stepValue f i.val j y = stepValue f i.val j (honestWord f sk i j)

theorem honest_continuation (f : HashTable) (sk : Words) (i : Fin 34) (j n : ℕ) :
...
theorem endpoint_match (f : HashTable) (sk : Words) (i : Fin 34) (j : ℕ) (hj : j ≤ 255)
    (x : Word) (h : chainValue f i.val j (255 - j) x = endpoints f sk i) :
    x = honestWord f sk i j ∨ ∃ k, j ≤ k ∧
      ChainSecondPreimage f sk i k (chainValue f i.val j (k - j) x) := by
  have href : chainValue f i.val j (255 - j) (honestWord f sk i j) = endpoints f sk i := by
    rw [honest_continuation, Nat.add_sub_of_le hj]
    rfl
  rcases chain_merge f i.val j (255 - j) x (honestWord f sk i j) (h.trans href.symm) with
    heq | ⟨k, hk, hne, heq⟩
  · exact Or.inl heq
  · right
...
theorem same_message_forgery (f : HashTable) (sk : Words) (m : Message) (bits : List Bool)
    (hlen : bits.length = 4352) (hne : bits ≠ encode (signedWords f sk m))
    (he : reconstructedWords f m bits = endpoints f sk) :
    ChainForgeryEvent f sk m bits := by
