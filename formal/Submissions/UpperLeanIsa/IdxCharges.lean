...
* a fresh index answer has a given index with probability `1 / 2 ^ 128` (`idxPost_charge`).
...
/-- Averaging `a * 2 ^ (hashBits - 128)` over the `2 ^ hashBits` answers gives `a / 2 ^ 128`. -/
theorem inv_card_mul_pow (hidx : 128 ≤ hashBits) (a : ℝ≥0∞) :
    (Fintype.card (BitVec hashBits) : ℝ≥0∞)⁻¹ * (a * (2 ^ (hashBits - 128) : ℕ)) =
      a / 2 ^ 128 := by
  have hc : (Fintype.card (BitVec hashBits) : ℝ≥0∞) =
      2 ^ 128 * 2 ^ (hashBits - 128) := by
    rw [Fintype.card_bitVec, ← pow_add, Nat.add_sub_cancel' hidx]
    push_cast
    rfl
  have h2 : ((2 : ℝ≥0∞) ^ (hashBits - 128))⁻¹ * 2 ^ (hashBits - 128) = 1 :=
    ENNReal.inv_mul_cancel (by simp) (by simp)
  rw [hc, Nat.cast_pow, Nat.cast_ofNat, ENNReal.mul_inv (Or.inl (by simp)) (Or.inl (by simp)),
    div_eq_mul_inv]
  calc ((2 : ℝ≥0∞) ^ 128)⁻¹ * ((2 : ℝ≥0∞) ^ (hashBits - 128))⁻¹ *
        (a * 2 ^ (hashBits - 128))
      = a * ((2 : ℝ≥0∞) ^ 128)⁻¹ *
          (((2 : ℝ≥0∞) ^ (hashBits - 128))⁻¹ * 2 ^ (hashBits - 128)) := by ring
    _ = a * ((2 : ℝ≥0∞) ^ 128)⁻¹ := by rw [h2, mul_one]

/-- The number of answers with a given index is at most `2 ^ (hashBits - 128)`. -/
theorem card_idxOf_eq_le (hidx : 128 ≤ hashBits) (i : ℕ) :
  ...
  have h1 : (Finset.univ.filter fun w : BitVec hashBits => idxOf w = i) =
      Finset.univ.filter fun w : BitVec hashBits =>
        idxOf w ∈ ({i} : Finset ℕ).filter fun n => n < 2 ^ 128 := by
    ext w
...
/-- A fresh encoding answer has index `i` with probability `1 / 2 ^ 128`. -/
theorem idxPost_charge (hidx : 128 ≤ hashBits) (d' d : Cache) (u : EncInput)
    ...
    ∑ w : BitVec hashBits, (Fintype.card (BitVec hashBits) : ℝ≥0∞)⁻¹ *
      ...
      (if P.IdxPost d' d i then 1 else 0) + ((2 : ℝ≥0∞) ^ 128)⁻¹ := by
