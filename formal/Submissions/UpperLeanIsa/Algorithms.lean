...
def keygen : OracleComp Spec (PublicKey × Words) := do
  let sk ← tabulate (fun _ : Fin 34 => sampleBits 128)
  let endpoints ← tabulate (fun i : Fin 34 => chain i.val 0 255 (sk i))
  let pk ← root endpoints
  pure (pk, sk)

def sign (sk : Words) (m : Message) : OracleComp Spec (Option (List Bool)) := do
  let xs ← tabulate (fun i : Fin 34 => chain i.val 0 (digit m i) (sk i))
  pure (some (encode xs))

def verify (pk : PublicKey) (m : Message) (bits : List Bool) : OracleComp Spec Bool := do
  if bits.length ≠ 4352 then return false
  let xs := decode bits
  let endpoints ← tabulate (fun i : Fin 34 =>
    chain i.val (digit m i) (255 - digit m i) (xs i))
