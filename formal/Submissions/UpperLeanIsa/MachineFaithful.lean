...
* `machineSubmission`: the FLAT-42 layer scheme (`Flat.scheme`), the HL-TRI bytecode (`program`),
  memory `2 ^ 16`, the honest prover (`prover`) and the step count `253` (every completing run
  executes exactly `253` instructions, `totalSteps_eq`).
...
theorem faithful : machineSubmission.Faithful := by
  ...
  have hrun : simulateQ (unifFwdAnswerImpl f) (machineSubmission.honestRun pk m bits) =
    ...
    show simulateQ _ (prover pk m bits >>= fun L => (fun o : Option ℕ => o.isSome) <$>
      LeanIsa.runCost program (LeanIsa.loadInput pk m bits L) 253 Regs.initial) = _
  ...
  generalize simulateQ (unifFwdAnswerImpl f) (LeanIsa.runCost program
    (LeanIsa.loadInput pk m bits (imageF f pk m bits)) 253 Regs.initial) = X at hs hh ⊢
...
/-- **Cycles** for the machine submission: every completing run costs `1390`. -/
theorem machine_cycles : machineSubmission.CyclesAtMost claim := cycles machineSubmission rfl

/-- The claim constant. -/
theorem claim_eq : claim = 1390 := rfl
