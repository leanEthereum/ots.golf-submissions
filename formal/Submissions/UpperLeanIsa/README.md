# leanISA — 1433 cycles (HL-TRI-TM)

This root keeps HL-FLAT-A's security proofs unchanged (the scheme differs only in its tag and
root-metadata constant values) (42 chains of 3-bit and 4-bit
...
Every completing run executes 260 instructions (117 `BLAKE2S`):
`143 + 10·117 + 120 = 1433` cycles. The tag symbols and root metadata reuse existing
constant cells.
...
- `MachineCycles`: the tie gives `acc_13 = idx` and the layer product gives `Σ s = 106`, both
  hash-free. The run is exactly 260 steps and 1313 cycles.
