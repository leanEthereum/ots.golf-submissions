...
* `walk_group`: the one-level subtree of a Rice(1) group;
* `walk_leaf`, `walk_leafHi`: a leaf's straight ops, then its unconditional `JUMP`;
...
theorem walk_node (hR : ∀ s, R s → HoldsNH L s) {s tc zc t n c : ℕ}
    ...
    (ht : t < 2 ^ 16) (hs : s + 1 < sentinel) (h : Walk R L n s c) :
...
theorem walk_node_mk (hR : ∀ s, R s → HoldsNH L s) {s tc zc t n c : ℕ}
    (hset : cinstrAt s = .setc tc (tgtV t)) (hjmp : cinstrAt (s + 1) = .jump zc tc oneCell)
    (ht : t < 2 ^ 16) (hs : s + 1 < sentinel) (hr0 : R s) (hr1 : R (s + 1))
...
theorem unaryExit_rice (k : ℕ) {x : ℕ} (hx : x ≤ nU k) :
    unaryExit (rBase k) 18 (nU k) x = gBase k x := by
  unfold unaryExit
  split_ifs with h
  · rw [gBase_of_lt h]
...
theorem walk_group (hR : ∀ s, R s → HoldsNH L s) {k q n c : ℕ} (hk : k < 41 ∨ k = 42)
  ...
  obtain ⟨hr0, hr1, n1, c1, rfl, rfl, hw⟩ := walk_node hR (cinstrAt_gset hk hq)
    (cinstrAt_gjmp hk hq) (by omega) (by rw [sentinel]; omega) h
  by_cases h0 : Lx L (zbCell k) = 0
  · rw [if_pos h0] at hw
    exact ⟨0, by omega, ⟨hr0, hr1, iff_of_true h0 rfl⟩, n1, c1, by omega, by omega,
...
theorem walk_group_mk (hR : ∀ s, R s → HoldsNH L s) {k q b n c : ℕ} (hk : k < 41 ∨ k = 42)
  ...
  rw [leafSlot_eq hb2] at h
  have e0 : gBase k q + 2 + 7 * b = if Lx L (zbCell k) = 0 then gBase k q + 2 else gBase k q + 9 := by
    by_cases h0' : Lx L (zbCell k) = 0
    · rw [if_pos h0']; have := h0.mp h0'; omega
    · rw [if_neg h0']; have : b ≠ 0 := fun e => h0' (h0.mpr e); omega
  have hw := walk_node_mk hR (cinstrAt_gset hk hq) (cinstrAt_gjmp hk hq) (by omega)
    (by rw [sentinel]; omega) hr0 hr1 (h.cast rfl e0 rfl)
  exact hw.cast (by omega) rfl (by omega)
...
theorem leaf_split (k e : ℕ) :
    ∃ a, leafLen k e = a + 2 ∧
      leafOp k e a = .setc (leafJC k e) (tgtV (s0 k + min e 126 + 1)) ∧
