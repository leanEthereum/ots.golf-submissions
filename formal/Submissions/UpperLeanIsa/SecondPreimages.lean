...
def AnswerMatches (ξ : Record) (a : HashLocation) (u : BitVec hashBits) : Prop :=
  ...
  | .inr i => if i = 33 then u.setWidth 128 = (ξ.2 a).setWidth 128 else u = ξ.2 a
...
def matchingAnswers (ξ : Record) (a : HashLocation) : Finset (BitVec hashBits) :=
  ...
  | .inr i => if i = 33 then lowAnswers (ξ.2 a) else {ξ.2 a}
...
theorem matchingAnswers_card (ξ : Record) (a : HashLocation) :
  ...
  | inr i =>
    change (if i = 33 then lowAnswers (ξ.2 (.inr i)) else {ξ.2 (.inr i)}).card ≤ _
    split
