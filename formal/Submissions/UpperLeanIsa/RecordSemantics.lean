...
def Record.rootState (ξ : Record) (k : Fin 40) : BitVec 256 :=
  if h : k.val = 0 then 0 else ξ.2 (.inr ⟨k.val - 1, by have := k.isLt; omega⟩)

theorem Record.rootState_before (ξ : Record) (i : Fin 39) :
    ξ.rootState i.castSucc = ξ.rootBefore i := rfl

theorem Record.rootState_after (ξ : Record) (i : Fin 39) :
...
theorem rootValueFold_record_of_roots (f : HashTable) (ξ : Record)
    (hf : ∀ i : Fin 39, f (ξ.query (.inr i)) = ξ.2 (.inr i))
    (n j : ℕ) (hj : j + n = 39) :
    rootValueFold f (List.ofFn (fun k : Fin n => ξ.endpoint ⟨j + k.val, by have := k.isLt; omega⟩))
      (ξ.rootState ⟨j, by omega⟩) = ξ.rootState 39 := by
  induction n generalizing j with
  | zero =>
    have : j = 39 := by omega
    subst j
...
theorem rootValueFold_record (f : HashTable) (ξ : Record) (hf : Respects f ξ)
    (n j : ℕ) (hj : j + n = 39) :
    rootValueFold f (List.ofFn (fun k : Fin n => ξ.endpoint ⟨j + k.val, by have := k.isLt; omega⟩))
      (ξ.rootState ⟨j, by omega⟩) = ξ.rootState 39 :=
...
theorem rootValue_record_of_roots (f : HashTable) (ξ : Record)
  ...
  have h := rootValueFold_record_of_roots f ξ hf 39 0 (by omega)
  have hl : (fun k : Fin 39 => ξ.endpoint ⟨0 + k.val, by have := k.isLt; omega⟩) = ξ.endpoint := by
    funext k
    simp only [Nat.zero_add]
  rw [hl] at h
  change rootValueFold f (List.ofFn ξ.endpoint) 0 = ξ.2 (.inr 38) at h
