...
theorem encode_decode (bits : List Bool) (h : bits.length = 4352) :
    encode (decode bits) = bits := by
  have hlen (i : Fin 34) : ((bits.drop (128 * i.val)).take 128).length = 128 := by
  ...
  calc
    _ = (List.ofFn (fun i : Fin 34 => (bits.drop (128 * i.val)).take 128)).flatten :=
      congrArg List.flatten (congrArg List.ofFn (funext fun i => bits_ofBits _ (hlen i)))
    _ = bits := join_slices 128 34 bits h
