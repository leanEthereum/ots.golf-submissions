...
theorem trunc_val_ch (ξ : Rec) (k : Fin 54) (t : Fin 18) :
    trunc (graph.evalRec ξ (ch k t).fin) = trunc (ξ.2 (ch k t).fin) := by
  rw [trunc_evalRec, val_ch]
  rfl

theorem val_cv (ξ : Rec) (k : Fin 54) (t : Fin 18) : val ξ (cv k t) = trunc (ξ.2 (ch k t).fin) := by
  unfold val
  rw [evalRec_apply_fin]
  simp only [kindOf, NodeKind.value]
  refine (cast_cast_eq _ _ _).trans ?_
...
theorem trunc_val_cv (ξ : Rec) (k : Fin 54) (t : Fin 18) :
...
theorem val_gc (ξ : Rec) (j : Fin 18) :
    val ξ (gc j) = tw (gh j) ++ cat3 (trunc (ξ.2 (ch (chainOf j 0) 17).fin))
      (trunc (ξ.2 (ch (chainOf j 1) 17).fin)) (trunc (ξ.2 (ch (chainOf j 2) 17).fin)) := by
  unfold val
  rw [evalRec_apply_fin]
  simp only [kindOf, NodeKind.value]
  refine (cast_cast_eq _ _ _).trans ?_
  show tw (gh j) ++ cat3 (trunc (graph.evalRec ξ (cv (chainOf j 0) 17).fin))
    (trunc (graph.evalRec ξ (cv (chainOf j 1) 17).fin))
    (trunc (graph.evalRec ξ (cv (chainOf j 2) 17).fin)) = _
...
theorem val_rc (ξ : Rec) : val ξ rc = tw rh ++ cat18 fun l => trunc (ξ.2 (gh l).fin) := by
  unfold val
  rw [evalRec_apply_fin]
  simp only [kindOf, NodeKind.value]
  refine (cast_cast_eq _ _ _).trans ?_
  show tw rh ++ cat18 (fun l => trunc (graph.evalRec ξ (gv l).fin)) = _
  exact congrArg (fun a => tw rh ++ cat18 a) (funext fun l => trunc_val_gv ξ l)
...
/-- The input of a hash node has length 144 (chains), 400 (groups) or 2320 (root). -/
theorem len_hashParent_cases {h p : Name} (hp : hashParent h = some p) :
    p.len = 144 ∨ p.len = 400 ∨ p.len = 2320 := by
  cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;> subst hp <;>
    simp [Name.len]
...
theorem tagNat_val {h p : Name} (hp : hashParent h = some p) (ξ : Rec) :
  ...
  · rw [val_rc]; exact tagNat_tw_append (n := 2304) _ _
...
/-- A keygen point is not an index query: its length is 144, 400 or 2320, never 384. -/
...
theorem trunc_cat18 (a : Fin 18 → BitVec 128) : trunc (cat18 a) = a 17 := by
  unfold cat18
  rw [trunc_cast]
  unfold trunc
  rw [BitVec.setWidth_append, dif_pos le_rfl, BitVec.setWidth_eq]
...
/-- A tweaked concatenation of eighteen values: the low 128 bits are the last value. -/
theorem trunc_of_tw_cat18_eq {a : BitVec 16} {b : Fin 18 → BitVec 128} {u : BitVec 2320}
    (e : a ++ cat18 b = u) : trunc u = b 17 :=
  (trunc_of_tw_append_eq (n := 2304) (by norm_num) e).trans (trunc_cat18 b)
