...
* if it is fresh, the answer is accepted with probability `numCuts / 2 ^ idxBits` and then loses
...
`|rowAcc| + NF · numCuts / 2 ^ idxBits`, where `NF` is the number of untried fresh nonces. Any `ρ`
...
theorem frac_acc (hidx : idxBits ≤ hashBits) (hM : numCuts ≤ 2 ^ idxBits) :
    ∑ w : BitVec hashBits, (Fintype.card (BitVec hashBits) : ℝ≥0∞)⁻¹ *
      (if idxOf w < numCuts then (1 : ℝ≥0∞) else 0) = (numCuts : ℝ≥0∞) / 2 ^ idxBits := by
  rw [← Finset.mul_sum, Finset.sum_boole]
  have h1 : (Finset.univ.filter fun w : BitVec hashBits => idxOf w < numCuts) =
      Finset.univ.filter fun w : BitVec hashBits => idxOf w ∈ Finset.range numCuts := by
    ext w; simp
  rw [h1, card_idxOf_mem hidx _ (fun n hn => lt_of_lt_of_le (Finset.mem_range.1 hn) hM),
    Finset.card_range, Nat.cast_mul, inv_card_mul_pow hidx]

theorem frac_split (w : BitVec hashBits) :
    (if idxOf w < numCuts then (1 : ℝ≥0∞) else 0) +
      (if idxOf w < numCuts then (0 : ℝ≥0∞) else 1) = 1 := by
  split_ifs <;> simp
...
theorem ind_le_rowBad {d : Cache} {m : Message} {η : Nonce} {w : BitVec hashBits}
    (hi : idxOf w < numCuts) (hw : d (encQuery (m ++ η)) = some w) :
...
theorem signRhoLoop_bound (hM' : numCuts ≤ 2 ^ idxBits) (hidx : idxBits ≤ hashBits)
    (m : Message) (d : Cache) {β J : Type} [Nonempty J]
    (kont : J → Option (Nonce × Fin numCuts) → OracleComp Spec β)
    (Fn : Option (Nonce × Fin numCuts) → Cache → ℝ≥0∞)
    ...
    (hρ : ∀ c : ℕ, (rowFresh d m).card ≤ c + trials → c ≤ (rowFresh d m).card →
      ((rowBad d m).card : ℝ≥0∞) + c * (((V d).card : ℝ≥0∞) / 2 ^ idxBits) ≤
        ρ * ((rowAcc d m).card + c * ((numCuts : ℝ≥0∞) / 2 ^ idxBits))) :
  ...
  have hnone : ∀ (d' : Cache) (b : ℕ) (tried : Finset Nonce), Cache.Sub d d' →
      ...
      Fn none d' + (if (none : Option (Nonce × Fin numCuts)) = none then lam * ρ else 0) ≤
        Φ d + κ * b + lam * ρ := by
  ...
  | succ k ih =>
    ...
    · simp only [signIdxLoop_succ m k tried hc] at hB ⊢
      ...
      set af : ℝ≥0∞ := (numCuts : ℝ≥0∞) / 2 ^ idxBits with haf
      set rf : ℝ≥0∞ := ∑ w : BitVec hashBits, (Fintype.card (BitVec hashBits) : ℝ≥0∞)⁻¹ *
        (if idxOf w < numCuts then (0 : ℝ≥0∞) else 1) with hrf
        ...
        · -- fresh encoding query
          ...
          have perw : ∀ w : BitVec hashBits,
              ...
              Φ d + κ * b + lam * ((if idxOf w ∈ V d then 1 else 0) +
                (if idxOf w < numCuts then 0 else ρ)) := by
            intro w
            by_cases hi : idxOf w < numCuts
          ...
          calc ∑ w : BitVec hashBits, (Fintype.card (BitVec hashBits) : ℝ≥0∞)⁻¹ *
              ...
              ≤ ∑ w : BitVec hashBits, (Fintype.card (BitVec hashBits) : ℝ≥0∞)⁻¹ *
                  (Φ d + κ * b + lam * ((if idxOf w ∈ V d then 1 else 0) +
                    (if idxOf w < numCuts then 0 else ρ))) :=
                Finset.sum_le_sum fun w _ => mul_le_mul_right (perw w) _
            _ = Φ d + κ * b + lam * (vf + ρ * rf) := by
                have hρw : ∀ w : BitVec hashBits, (if idxOf w < numCuts then 0 else ρ) =
                    ρ * (if idxOf w < numCuts then (0 : ℝ≥0∞) else 1) := fun w => by
                  split_ifs <;> simp
...
theorem signRho_bound (hM' : numCuts ≤ 2 ^ idxBits) (hidx : idxBits ≤ hashBits)
    (m : Message) (d : Cache) {β J : Type} [Nonempty J]
    (k : J → Option (Nonce × Fin numCuts) → OracleComp Spec β)
    (Fn : Option (Nonce × Fin numCuts) → Cache → ℝ≥0∞)
    (Φ : Cache → ℝ≥0∞) (hΦ : EncInvariant Φ) (κ lam ρ : ℝ≥0∞)
    (I : Cache → ℕ → Prop)
    (hI_fresh : ∀ c b q, I c b → c q = none → queryCost (.inr q) ≤ b →
