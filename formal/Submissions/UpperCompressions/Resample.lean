...
theorem updSrc_updSrc (ξ : Rec) (k : Fin 54) (b : BitVec 128) :
...
theorem fst_updSrc_self (ξ : Rec) (k : Fin 54) (b : BitVec 128) :
...
theorem child_cv_chainOf (j : Fin 18) (a : Fin 3) : child (cv (chainOf j a) 13) = some (gc j) := by
...
theorem child_gv_groupOf (l : Fin 6) (a : Fin 3) : child (gv (groupOf l a)) = some (ec l) := by
  rw [Name.child]
  congr 2
  ext
  simp only [groupOf]
...
theorem pkOf_updSrc (ξ : Rec) (k : Fin 54) (b : BitVec 128) : pkOf (updSrc ξ k b) = pkOf ξ := by
  unfold pkOf
  rw [snd_updSrc]
