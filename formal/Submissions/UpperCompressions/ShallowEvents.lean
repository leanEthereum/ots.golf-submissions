import Submissions.UpperCompressions.ShallowValues
...
theorem cat18_inj {a b : Fin 18 → BitVec 128} (h : cat18 a = cat18 b) : a = b := by
  unfold cat18 at h
...
theorem yv_rc (hy : graph.ReconEqs d (fins A) given y) (he : Evaluated A rc) :
  ...
  rw [yv_det hy he rfl (by simp)]
  show tw rh ++ cat18 (fun l => trunc (y (gv l).fin)) = _
  exact congrArg (fun a => tw rh ++ cat18 a)
    (funext fun l => trunc_eq_cast (graph_len_fin (gv l)) _)
...
theorem up {A : Finset Name} (hA : IsCut A) {ξ : Rec} {d : Cache}
  ...
  | _ n ih =>
    ...
    | gv j =>
      ...
      exact hne (congrFun (cat18_inj (append_inj (n := 2304) heq).2) j)
    | rc =>
...
#print axioms OptimalOTS.ShallowForest.events_none
#print axioms OptimalOTS.ShallowForest.events_ne
#print axioms OptimalOTS.ShallowForest.events_same
