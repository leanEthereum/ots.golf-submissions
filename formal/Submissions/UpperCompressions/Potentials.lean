...
import Submissions.UpperCompressions.Resample
import Submissions.UpperCompressions.Events
import Submissions.UpperCompressions.EncCharges
import Submissions.UpperCompressions.RowPotential
...
theorem probTrue_eq : probTrue (experiment forestScheme A) = E (run (experiment forestScheme A) ∅) g := by
...
theorem paperRowHyp : RowHyp where
  ...
  numCuts_le := by show 2 * 2 ^ 115 ≤ 2 ^ 128; norm_num
