...
/-!
# The signing loop with a submitted acceptance threshold

`signIdx m` is the signing loop returning the selected nonce and index instead of the
signature; `IndexedDag.Scheme.sign` is its image under encoding the revealed values.
...
/-! ### `IndexedDag.Scheme.sign` as the image of `signIdx` -/

theorem signLoop_eq_map (S : IndexedDag.Scheme numCuts) (x : S.graph.Assignment) (m : Message) :
  ...
  intro k
  induction k with
  | zero => intro tried; simp [IndexedDag.Scheme.signLoop, signIdxLoop]
  | succ k ih =>
    intro tried
    by_cases hc : 0 < (Finset.univ \ tried).card
    · rw [IndexedDag.Scheme.signLoop, signIdxLoop, dif_pos hc, dif_pos hc]
      simp only [map_bind]
      refine bind_congr fun j => ?_
      ...
      · simp
      · exact ih _
    · rw [IndexedDag.Scheme.signLoop, signIdxLoop, dif_neg hc, dif_neg hc]
      simp

/-- `IndexedDag.Scheme.sign` encodes the revealed values of the index found by `signIdx`. -/
theorem sign_eq_map (S : IndexedDag.Scheme numCuts) (x : S.graph.Assignment) (m : Message) :
    S.sign x m =
      (Option.map fun r : Nonce × Fin numCuts => (r.1, S.graph.encode (S.sets r.2) x)) <$>
        signIdx m :=
