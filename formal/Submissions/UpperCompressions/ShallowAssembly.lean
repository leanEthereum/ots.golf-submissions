...
```
probTrue (IndexedDag.experiment forestScheme A) ≤ 2 ε (B - 995),  ε = 2 ^ (-128).
```
...
private theorem cutIndex_zero_lt : 0 < IndexedAnalysis.numCuts := by
  norm_num [IndexedAnalysis.numCuts]

private theorem E_run_keygen_indexed
    (S : IndexedDag.Scheme numCuts) (T : S.graph.Tagging)
...
theorem E_run_keygen_forest
  ...
  rw [E_run_keygen_indexed forestScheme tagging g']
...
theorem E_run_experiment (g' : Bool × Cache → ℝ≥0∞) :
    E (run (IndexedDag.experiment forestScheme A) ∅) g' =
...
theorem costAtMost_rest_forest {B : ℕ} (hB : CostAtMost (IndexedDag.experiment forestScheme A) B) :
    995 ≤ B ∧ ∀ ξ : Rec, CostAtMost (rest A (pkOf ξ, graph.evalRec ξ)) (B - 995) := by
  rw [experiment_eq,
    ← IndexedDag.Scheme.keygenProxy_keygen forestScheme ⟨0, cutIndex_zero_lt⟩] at hB
  obtain ⟨h1, h2⟩ := costAtMost_keygen_bind
    (forestScheme.keygenProxy ⟨0, cutIndex_zero_lt⟩) (rest A) hB
  refine ⟨?_, fun ξ => ?_⟩
...
theorem keygen_le {B : ℕ} (hB : CostAtMost (IndexedDag.experiment forestScheme A) B) : 995 ≤ B :=
  (costAtMost_rest_forest A hB).1

theorem main_bound {B : ℕ} (hB : CostAtMost (IndexedDag.experiment forestScheme A) B) (hB' : B ≤ 2 ^ 127) :
    probTrue (IndexedDag.experiment forestScheme A) ≤ κ * ((B - 995 : ℕ) : ℝ≥0∞) := by
  obtain ⟨h995, hrest⟩ := costAtMost_rest_forest A hB
  ...
  calc ∑ ξ : Rec, w * E (run (rest A (pkOf ξ, graph.evalRec ξ)) (kc ξ)) g
    ...
    _ = ∑ pk, E (run (A.choose pk) ∅) (fun p => FA A pk p.1 p.2) := by
        ...
        refine Finset.sum_congr rfl fun ξ hξ => ?_
        rw [mem_fiberA_asm hξ]
    _ ≤ ∑ pk, κ * sumW (fiberA pk) * ((B - 995 : ℕ) : ℝ≥0∞) := by
        refine Finset.sum_le_sum fun pk _ => ?_
        refine stageA_master A pk (B - 995) ((Nat.sub_le B 995).trans hB') fun ξ hξ => ?_
        have h := hrest ξ
