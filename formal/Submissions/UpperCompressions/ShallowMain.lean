import Submissions.UpperCompressions.ShallowAssembly
...
attribute [local irreducible] IndexedDag.experiment forestScheme
...
theorem kappa_mul_lt {B : ℕ} (h995 : 995 ≤ B) :
    κ * ((B - 995 : ℕ) : ℝ≥0∞) < (B : ℝ≥0∞) / 2 ^ securityBits := by
  rw [kappa_eq]
  show _ < (B : ℝ≥0∞) / 2 ^ 127
...
/--
info: 'OptimalOTS.ShallowForest.forestScheme_secure' depends on axioms: [propext, Classical.choice, Quot.sound]
-/
#guard_msgs in
#print axioms ShallowForest.forestScheme_secure

/--
info: 'OptimalOTS.ShallowForest.forestScheme_verifyCost' depends on axioms: [propext, Classical.choice, Quot.sound]
-/
#guard_msgs in
#print axioms ShallowForest.forestScheme_verifyCost
