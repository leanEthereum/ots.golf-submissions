...
/-- The submitted smaller acceptance family meets every row-potential hypothesis. -/
theorem candidateRowHyp : RowHyp where
  nonce_eq := by norm_num [nonceBits, idxBits]
  idx_le := by norm_num [idxBits, hashBits]
  two_le := by norm_num [numCuts]
  numCuts_le := by norm_num [numCuts, idxBits]
  trial_le := by norm_num [trials, signBudget, idxCost, blockCost, msgBits,
    nonceBits, blockBits, idxBits]

#print axioms candidateRowHyp
#print axioms psi_charge
#print axioms psi_dom
