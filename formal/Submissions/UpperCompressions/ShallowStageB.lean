import Submissions.UpperCompressions.ShallowPotentials
import Submissions.UpperCompressions.IndexedReconstruct
...
theorem stB_support (pk : PublicKey) (m₁ : Message) (st : A.State) (σ : Option Signature)
  ...
  simp only [Set.mem_iUnion] at hp
  obtain ⟨⟨ok, c₂⟩, h₂, hp⟩ := hp
  obtain ⟨hsub₂, hver⟩ := OptimalOTS.IndexedAnalysis.verify_support forestScheme pk m₂ σ₂ c₁ ⟨ok, c₂⟩ h₂
...
theorem events_stB (ξ : Rec) (r : Option (Nonce × Fin numCuts)) (m₁ : Message) (st : A.State)
  ...
  · -- signing failed: everything is hidden
    ...
    · left
      ...
      exact hh
  · by_cases hji : (⟨idxOf w, hi⟩ : Fin numCuts) = i
